...
class course_viewer(tk.Tk):
    def __init__(self, course):
        ...
        self.notes_list_box = tk.Listbox(
            self.frame,
            width=50,
            yscrollcommand=self.scrollbar.set,
            font=("Courier New", 10),
        )
        self.notes_list_box.pack(expand=True, fill=tk.Y)
        self.scrollbar.config(command=self.notes_list_box.yview)
        ...
        if nts:
            for nt in nts:
                self.notes_list_box.insert(tk.END, nt[0])
